...
async def comprar_contas_list(c: Client, m: CallbackQuery):
    ...
    await m.edit_message_text(
        f"""<b>🎟️ Comprar Contas Premium</b>
...
async def comprar_contas(c: Client, m: CallbackQuery):
    ...
    total = f"\n\n<b>🧿 Total de Contas Premium</b>: {sum([int(x[1]) for x in contas])}" if contas else ""
    ...
    await m.edit_message_text(
        f"""<b>👾 Comprar CONTAS PREMIUM Unitário</b>
<i>- Qual o tipo de CONTAS PREMIUM que você deseja comprar?</i>

<b>⚠️ Caso queira contas premium específicas, pesquise via Inline no bot abaixo🔍</b>
...
async def buy_off_contas(c: Client, m: CallbackQuery):
    ...
    kb = InlineKeyboardMarkup(
                    inline_keyboard=[
                        [
                            InlineKeyboardButton(
                                text="💳 Compre as melhores Contas Premium",url=f"https://t.me/metralhastore_bot"
                            ),
                        ],
                    ]
                )
