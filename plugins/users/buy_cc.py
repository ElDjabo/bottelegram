...
[
     InlineKeyboardButton("🔍 Procurar", callback_data="procurar"),
    ...
    await m.edit_message_text(
        f"""<a href='https://i.ibb.co/bJgz6Ly/IMG-20230711-113223-015.jpg'</a><b>💳 Comprar GG's</b>
...
Validade: SISTEMA ANTI ROUBOUS
Cvv: ***"""

        base_ml = f"""<b>Cartão:</b> <i>{number[0:6]}**********</i>
<b>Validade:</b> SISTEMA ANTI ROUBOUS
<b>Cvv:</b> <i>***</i>
...
async def comprar_ccs(c: Client, m: CallbackQuery):
    ...
    if not levels_list:
        return await m.answer(
            "⚠️ Não há GG's disponíveis no momento, tente novamente mais tarde.",
            show_alert=True,
        )
...
<i>
GGS DE QUALIDADE JA DISPONÍVEL PARA COMPRAS! 💳

ESTOQUE:

INFINITE 498408, APROVANDO INGRESSO,VINHFO FACIL, IFOOD E ALIEXPRESS DIRETO E GPAY ✅💳|

CLASSIC 485464, APROVANDO ALIEXPRESS E 99 DIRETO! ✅💳|

ELO 650507 PAGANDO FATURA VIVO! E POSSIVELMENTE SUBINDO MERCADO LIVRE! ✅💳|

| UTILIZE /PIX PARA ADICIONAR SALDO NA STORE ❄️ |

STORE: @HUNTERSTOREGGBOT

SUPORTE PARA TROCAS DE GG DIE: @huntercenter7</i>
...
async def buy_final(c: Client, m: CallbackQuery):
    ...
    if not ccs_list:
        return await m.answer("⚠️ Não há GG's disponíveis deste nível.")
    ...
    if gates_is_on:
            ...
            if live_or_die[0]:  # caso venha cc live
                ...
                kb = InlineKeyboardMarkup(
                    inline_keyboard=[
                        [
                            InlineKeyboardButton(
                                text="🔕 Não passou? Peça reembolso @huntercenter7",
                ...
                kb = InlineKeyboardMarkup(
                    inline_keyboard=[
                        [
                            InlineKeyboardButton(
                                text="💳 Compre as melhores GG's",url=f"https://t.me/{BOT_LINK}"
                            ),
                        ],
                    ]
                )
        ...
        if (live == 0) and gates_is_on:
            txt = "❗️ Não consegui achar GG's lives deste nível na minha database. Tente novamente com outro nível ou bin."
            kb = InlineKeyboardMarkup(
                inline_keyboard=[
                    [
                        InlineKeyboardButton(
...
async def buy_off(c: Client, m: CallbackQuery):
    ...
    if not selected_cc:
        return await m.answer("⚠️ Sem GG's disponiveis para este nivel.", show_alert=True)
