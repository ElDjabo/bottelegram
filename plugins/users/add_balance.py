...
<b>⚠️ Se vc já fez o pagamento, envie o comprovante para @huntercenter7
{details}""",
        reply_markup=kb,
    )
...
- Gift card: <code>{gift}</code>"""
    ...
    kb = InlineKeyboardMarkup(
                    inline_keyboard=[
                        [
                            InlineKeyboardButton(
                                text="💳 Compre as melhores GG's",url="https://t.me/HUNTERSTOREGGBOT."
                            ),
                        ],
                    ]
                )
    mention = m.from_user.first_name
    base = f"""<a href='https://t.me/Savitarstoresbot'>&#8204</a>🎁 {mention} resgatou um gift card de <b>R${value}</b>
<b>Obrigado pela a preferência</b>
- Gift card: <code>{gift[0:6]}</code>\n<a href='https://t.me/HUNTERSTOREGGBOT.'>Recarregar</a>"""
    ...
    if refer:
        quantity = math.floor((value / 100) * 20)  # 20% normalizado para int.
        if quantity > 0:
